...
func (ks *KeyStore) SignTxPermissionId(a Account, tx *core.Transaction) (*core.Transaction, error) {
	// Look up the key to sign with and abort if it cannot be found
	ks.mu.RLock()
	defer ks.mu.RUnlock()
	...
	signatures, err := stringArrayToByteSlice(string(tx.GetSignature()[0]))
	if err != nil {
		return nil, err
	}
	h256h := sha256.New()
